...
UPDATE task_states
    ...
    WHERE
        ...
        forms.slug = sqlc.arg('form_slug') AND
        tasks.slug = sqlc.arg('task_slug') AND
...
UPDATE task_states
    SET idx =
        CASE WHEN task_id = sqlc.arg('task_id_1') THEN (
            SELECT idx FROM task_states WHERE task_id = sqlc.arg('task_id_2')
        ) WHEN task_id = sqlc.arg('task_id_2') THEN (
            SELECT idx FROM task_states WHERE task_id = sqlc.arg('task_id_1')
        ) ELSE idx
        END
    WHERE task_id = sqlc.arg('task_id_1') OR task_id = sqlc.arg('task_id_2');

-- name: RemoveTask :exec
UPDATE task_states
    SET idx = -1
    WHERE task_id = sqlc.arg('task_id_1');
...
UPDATE task_states
    ...
    WHERE
        creators.username = sqlc.arg('creator_username') AND
        forms.slug = sqlc.arg('form_slug') AND
